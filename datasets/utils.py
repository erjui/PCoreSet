...
class DatasetWrapper(TorchDataset):
    def __init__(self, data_source, input_size, transform=None, is_train=False,
                 return_img0=False, k_tfm=1, return_idx=False):
    ...
    def __getitem__(self, idx):
        ...
        if self.return_idx:
            output['idx'] = idx
            return output['img'], output['label'], output['idx']
...
def build_data_loader(
    ...
    # Handle duplicating samples if needed to avoid dropping the last batch
    modified_data_source = data_source
    if drop_last and duplicate_if_needed and data_source:
        num_samples = len(data_source)
        num_batches = (num_samples + batch_size - 1) // batch_size  # Ceiling division
        target_size = num_batches * batch_size

        if target_size > num_samples:
            # Create a new list with all original samples
            modified_data_source = list(data_source)

            # Calculate how many more samples we need
            num_to_duplicate = target_size - num_samples

            # Add duplicated samples from the beginning
            for i in range(num_to_duplicate):
                modified_data_source.append(data_source[i % num_samples])

    # Build data loader
    def worker_init_fn(worker_id):
        """Initialize worker to use all CPU cores"""
        os.sched_setaffinity(0, range(os.cpu_count()))

    data_loader = torch.utils.data.DataLoader(
        dataset_wrapper(modified_data_source, input_size=input_size, transform=tfm, is_train=is_train, return_idx=return_idx),
