...
def subset_npercent_dataset(ds, percent=1, balanced=True):
    ...
    # Handle 100% case
    if percent >= 1.0:
        return ds, copy(ds)  # Return full dataset as labeled, empty dataset as unlabeled

    x_train_labels = np.array([y for _, y in ds.samples])
    ...
    ds.samples = [(x[0], int(x[1])) for x in tqdm(train_samples, desc="Processing labeled samples")]
    unbl_ds.samples = [(x[0], int(x[1])) for x in tqdm(unlbl_samples, desc="Processing unlabeled samples")]

    return ds, unbl_ds

class ImageNetPercent():

    dataset_dir = 'imagenet'

    def __init__(self, root, train_transform, test_transform, percent=1):
        ...
        self.train_x, self.train_u = subset_npercent_dataset(self.train_x, percent=percent)

        print(f"Train x: {len(self.train_x.samples)}, Train u: {len(self.train_u.samples)}")
